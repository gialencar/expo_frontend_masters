...
const AddNewPaletteModal = ({ navigation }) => {
  ...
  const handleSwitchToggle = (state, colorName) => {
    if (state) {
      setSelectedColors((colors) => [
        ...colors,
        COLORS.find((c) => c.colorName === colorName),
      ]);
    } else {
      setSelectedColors((colors) =>
        colors.filter((color) => color.colorName !== colorName),
      );
    }
  };
        ...
        renderItem={({ item }) => (
          <View style={styles.switch}>
            ...
            <Switch
              ...
              onValueChange={(state) =>
                handleSwitchToggle(state, item.colorName)
              }
            />
          </View>
        )}
